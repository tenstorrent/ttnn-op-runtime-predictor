...
auto parse_args(int argc, char** argv){
    ...
    if(strcmp(argv[3], "--op-category") != 0){
        throw std::invalid_argument("Wrong second argument, should be --op-category <op category>");
    }
    if(strcmp(argv[5], "--dataset") != 0){
        throw std::invalid_argument("Wrong third argument, should be --dataset <path/to/dataset.csv>");
    }
    if(strcmp(argv[7], "--input-dim") != 0){
        throw std::invalid_argument("Wrong fourth argument, should be --input-dim <mlp input dimension>");
    }

    //optional args
    std::optional<std::vector<int>> hidden_layers;
    std::optional<int> batch_size, max_iterations;
    std::optional<double> learning_rate, momentum_decay, squared_gradient_decay, epsilon, tolerance;
...
int main(int argc, char** argv){
    try{
        auto [op_name, op_category, dataset_filepath, input_dim, hidden_layers, batch_size, learning_rate, momentum_decay, squared_gradient_decay, epsilon, max_iterations, tolerance] = parse_args(argc, argv);
