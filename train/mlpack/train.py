...
def read_dataset(file_path, filter_condition):
    ...
    with open(file_path, 'r') as file:
        ...
        for row in reader:
            if row and row[:4] == filter_condition:  # Apply filter condition
...
def train_mlp_regressor(inputs, outputs):
    ...
    model = MLPRegressor(hidden_layer_sizes=(128, 128, 128), max_iter=1000, solver="adam", batch_size=1024, random_state=seed, verbose=1, learning_rate_init=0.0001)
    pipeline = Pipeline([
        ('scaler', scaler),
        ('mlp', model)
    ])
...
if __name__ == "__main__":
    file_path = "flattened_results.csv"  # Change this to your actual file path
    filter_condition = ["BLOCK", "ROW_MAJOR", "WIDTH", "ROW_MAJOR"]  # Change filter condition as needed
    ...
    joblib.dump(model, 'reshard_model.pkl')
    pkl_to_csv("block_height.pkl")